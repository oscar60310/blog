...
通常我們會把內部的服務像是 Databases 或應用程式放在 Private Subnets，這些機器不能直接由 Public IP 存取，而是藉由 Load Balancers (ALB, ELB) 將流量導向內部的機器，這樣可以保護我們的服務不受外部攻擊，只讓真正需要對外的服務能夠被存取。<!--truncate-->如下圖所示（通常 Applications 和 Databases 不會放在同一個 Subnet)：
...
:::info
如果您打算在生產環境使用 Teleport， 請在正式部署前查看 [Teleport Production Guide](https://goteleport.com/docs/production)，裡面有非常詳細的教學。
...
<figcaption>Session recording</figcaption>
</figure>
...
<figcaption>圖片來自 https://goteleport.com/docs/kubernetes-access/controls/</figcaption>
</figure>
