...
:::info
Kube-proxy 部分可以參考 [GKE](https://cloud.google.com/kubernetes-engine/docs/concepts/network-overview?authuser=0#kube-proxy) 的說明文件。
...
前半段和 ClusterIP 很像，只不過多了一層 KUBE-NODEPORTS Chain，一樣最後會經由 DNAT 到 Pod。
...
:::info
當從 Node1 內部打 Port 30000 時，會照正常的流程走，如同 Policy = Cluster 一樣。
...
<figcaption>Azure LB health probes 設定</figcaption>
</figure>
